...
@pytest.mark.asyncio
async def test_baixar_xml_busca_da_spedy_quando_esse_e_o_provedor(db_session, monkeypatch):
    ...
    try:
        ...
        assert resposta.status_code == 200
        assert resposta.content == b"<NFSe>da-spedy</NFSe>"
        assert "NFSe_1_1.xml" in resposta.headers["content-disposition"]
    finally:
        app.dependency_overrides.clear()


@pytest.mark.asyncio
async def test_baixar_xml_devolve_404_quando_spedy_falha_ao_buscar_xml(db_session, monkeypatch):
    from app.models import ProvedorEmissao
