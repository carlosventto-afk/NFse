...
async def _duas_empresas(db_session) -> tuple[Emissao, Usuario, Usuario]:
    _empresa_a, usuario_a = await _empresa_com_usuario(
    ...
    emissao_de_a = Emissao(
        empresa_id=usuario_a.empresa_id, origem=OrigemEmissao.manual,
...
async def test_listagem_nao_mostra_emissao_de_outra_empresa(db_session):
    emissao_de_a, usuario_a, usuario_b = await _duas_empresas(db_session)
    ...
    try:
        ...
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            de_b = await client.get(
                "/emissoes", headers={"Authorization": f"Bearer {criar_token(usuario_b)}"}
            )
            de_a = await client.get(
                "/emissoes", headers={"Authorization": f"Bearer {criar_token(usuario_a)}"}
            )
    ...
    try:
        ...
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            de_b = await client.get(
                ...
                headers={"Authorization": f"Bearer {criar_token(usuario_b)}"},
            )
            de_a = await client.get(
                f"/emissoes/{emissao_de_a.id}/xml",
                headers={"Authorization": f"Bearer {criar_token(usuario_a)}"},
            )
